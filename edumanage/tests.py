...
class LessonModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email='teacher@example.com', password='pass123')
        self.course = Course.objects.create(
            title='Physics',
            owner=self.user
        )
    ...
    def test_course_deletion_removes_lessons(self):
        lesson = Lesson.objects.create(course=self.course, title='Lesson', owner=self.user)
        self.course.delete()
